...
macro_rules! define_matrix { ($name:ident, $nx:expr, $ny:expr) => {
    ...
    impl<T> Add<&$name<T>> for &$name<T> where T : Copy + Add<Output = T> {
        type Output = $name<T>;
    ...
    impl<T> Sub<&$name<T>> for &$name<T> where T : Copy + Sub<Output = T> {
        type Output = $name<T>;
    ...
    impl<T> Mul<T> for &$name<T> where T : Copy + Default + Mul<T, Output=T> {
        type Output = $name<T>;
    ...
    impl<T> Mul<T> for $name<T> where T : Copy + Default + Mul<T, Output=T> {
        type Output = $name<T>;
        fn mul(self, other : T) -> Self::Output { &self * other }
    }
    ...
    impl<T> MulAssign<T> for $name<T> where T : Copy + Default + Mul<T, Output=T> {
...
macro_rules! define_square_matrix { ($name:ident, $n:expr) => {
    ...
    impl<T> Dot<&$name<T>> for &$name<T> where T : Copy + Default + Add<T, Output=T> + Mul<T, Output=T> {
        type Output = $name<T>;
    ...
    impl<T> Dot<Self> for $name<T> where T : Copy + Default + Add<T, Output=T> + Mul<T, Output=T> {
        type Output = $name<T>;
        fn dot(self, other: Self) -> Self { Dot::dot(&self, &other) }
    }

    impl<T> Mul<&$name<T>> for &$name<T> where T : Copy + Default + Add<T, Output=T> + Mul<T, Output=T> {
        type Output = $name<T>;
        fn mul(self, other : &$name<T>) -> Self::Output { Dot::dot(self, other) }
    }
...
impl Matrix44<f32> {
    ...
    pub fn transform_3d(&self, v: &Vector3<f32>) -> Vector3<f32> {
        ...
        for i in 0..3 {
            values[i] = v.x * self.values[i][0] + v.y * self.values[i][1] + v.z * self.values[i][2] + self.values[i][3];
        }
    ...
    pub fn transform_4d(&self, v: &Vector4<f32>) -> Vector4<f32> {
        ...
        Vector4 { x: values[0], y: values[1], z: values[2], w: values[3] }
    }
    ...
    fn set_inverted(&mut self) -> &mut Self {
        ...
        /* 行列式の計算 */
        let b11 =  a22 * (a33 * a44 - a43 * a34) - a23 * (a32 * a44 - a42 * a34) + a24 * (a32 * a43 - a42 * a33);
        let b12 = -a12 * (a33 * a44 - a43 * a34) + a13 * (a32 * a44 - a42 * a34) - a14 * (a32 * a43 - a42 * a33);
        let b13 =  a12 * (a23 * a44 - a43 * a24) - a13 * (a22 * a44 - a42 * a24) + a14 * (a22 * a43 - a42 * a23);
        let b14 = -a12 * (a23 * a34 - a33 * a24) + a13 * (a22 * a34 - a32 * a24) - a14 * (a22 * a33 - a32 * a23);

        let b21 = -a21 * (a33 * a44 - a43 * a34) + a23 * (a31 * a44 - a41 * a34) - a24 * (a31 * a43 - a41 * a33);
        let b22 =  a11 * (a33 * a44 - a43 * a34) - a13 * (a31 * a44 - a41 * a34) + a14 * (a31 * a43 - a41 * a33);
        let b23 = -a11 * (a23 * a44 - a43 * a24) + a13 * (a21 * a44 - a41 * a24) - a14 * (a21 * a43 - a41 * a23);
        let b24 =  a11 * (a23 * a34 - a33 * a24) - a13 * (a21 * a34 - a31 * a24) + a14 * (a21 * a33 - a31 * a23);

        let b31 =  a21 * (a32 * a44 - a42 * a34) - a22 * (a31 * a44 - a41 * a34) + a24 * (a31 * a42 - a41 * a32);
        let b32 = -a11 * (a32 * a44 - a42 * a34) + a12 * (a31 * a44 - a41 * a34) - a14 * (a31 * a42 - a41 * a32);
        let b33 =  a11 * (a22 * a44 - a42 * a24) - a12 * (a21 * a44 - a41 * a24) + a14 * (a21 * a42 - a41 * a22);
        let b34 = -a11 * (a22 * a34 - a32 * a24) + a12 * (a21 * a34 - a31 * a24) - a14 * (a21 * a32 - a31 * a22);

        let b41 = -a21 * (a32 * a43 - a42 * a33) + a22 * (a31 * a43 - a41 * a33) - a23 * (a31 * a42 - a41 * a32);
        let b42 =  a11 * (a32 * a43 - a42 * a33) - a12 * (a31 * a43 - a41 * a33) + a13 * (a31 * a42 - a41 * a32);
        let b43 = -a11 * (a22 * a43 - a42 * a23) + a12 * (a21 * a43 - a41 * a23) - a13 * (a21 * a42 - a41 * a22);
        let b44 =  a11 * (a22 * a33 - a32 * a23) - a12 * (a21 * a33 - a31 * a23) + a13 * (a21 * a32 - a31 * a22);
